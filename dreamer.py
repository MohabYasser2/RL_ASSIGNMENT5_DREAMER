...
"""
Core training agent (Dreamer).

This module composes the world model (encoder/decoder/prior/posterior/recurrent)
with actor and critic to implement the Dreamer training loop. Methods are:
- `worldModelTraining`: trains the latent/world model from replay samples
- `behaviorTraining`: trains the actor & critic in imagination rollouts
- `environmentInteraction`: collects data into the `ReplayBuffer` and (optionally)
    returns evaluation metrics / saves videos.

When updating training logic, preserve the counters `totalEnvSteps`,
`totalGradientSteps`, and `totalEpisodes` so that checkpoint/resume works.
"""

from networks import RecurrentModel, PriorNet, PosteriorNet, RewardModel, ContinueModel, EncoderConv, DecoderConv, Actor, DiscreteActor, Critic
from utils import computeLambdaValues, Moments
from buffer import ReplayBuffer
import imageio

# Optional wandb logging (if installed)
try:
        import wandb
...
class Dreamer:
    def __init__(self, observationShape, isDiscrete, actionSize, actionLow, actionHigh, device, config):
        ...
        if isDiscrete:
            self.actor = DiscreteActor(self.fullStateSize, actionSize, device, config.actor).to(self.device)
        else:
        ...
        self.buffer         = ReplayBuffer(observationShape, actionSize, config.buffer, device, is_discrete=isDiscrete)
        ...
        # Counters that must be preserved across checkpoints/resume
    ...
    def worldModelTraining(self, data):
        # Encode raw observations (batch, seq, C,H,W) -> (batch, seq, encodedSize)
        ...
        for t in range(1, self.config.batchLength):
            # Convert discrete actions to one-hot encoding for recurrent model
            if self.isDiscrete:
                action_input = F.one_hot(data.actions[:, t-1], self.actionSize).float()
            else:
                action_input = data.actions[:, t-1]
            
            recurrentState              = self.recurrentModel(previousRecurrentState, previousLatentState, action_input)
        ...
        # Return detached full states (for behavior training) and metrics dict.
        return fullStates.view(-1, self.fullStateSize).detach(), metrics


    def behaviorTraining(self, fullState):
        # `fullState` is (batch*batchLength, fullStateSize). Split to recurrent & latent
        ...
        for _ in range(self.config.imaginationHorizon):
            ...
            # Convert discrete actions to one-hot encoding for recurrent model
            if self.isDiscrete:
                action_input = F.one_hot(action, self.actionSize).float()
            else:
                action_input = action
            
            recurrentState = self.recurrentModel(recurrentState, latentState, action_input)
    ...
    def environmentInteraction(self, env, numEpisodes, seed=None, evaluation=False, saveVideo=False, filename="videos/unnamedVideo", fps=30, macroBlockSize=16):
        # Interact with `env` for `numEpisodes` episodes. When `evaluation=True` the
        # collected transitions are NOT stored in the replay buffer. When `saveVideo=True`
        # the first episode frames are recorded and saved.
        scores = []
        for i in range(numEpisodes):
            ...
            if self.isDiscrete:
                action = torch.zeros(1, dtype=torch.long).to(self.device)
            else:
            ...
            while not done:
                recurrentState      = self.recurrentModel(recurrentState, latentState, action if not self.isDiscrete else F.one_hot(action, self.actionSize).float())
                ...
                if self.isDiscrete:
                    actionNumpy = action.cpu().numpy().item()
                else:
                ...
                if done:
                    ...
                    # Log evaluation/interaction score to wandb if available
                    if _WANDB_AVAILABLE and evaluation:
                        wandb.log({"evaluation/episode_score": currentScore, "env/episode_steps": stepCount}, step=self.totalGradientSteps)
                    break
