"""
Network components used by the Dreamer agent.

Each class here implements a small, self-contained network used by the
world model (encoder/decoder/prior/posterior), the actor (continuous/discrete)
and the critic. Keep layer sizes and activations in sync with the config
files under `configs/`.
"""
...
class DiscreteActor(nn.Module):
    ...
    def forward(self, x, training=False):
        ...
        if training:
            ...
            # For evaluation, return the most likely action
